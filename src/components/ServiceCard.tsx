...
export default function ServiceCard({ title, desc, tags, imageSrc }: ServiceCardProps) {
  ...
  return (
    <div className="relative w-full rounded-[26px] p-[2px] border-sweep shadow-[0_0_40px_rgba(0,212,255,0.1),0_0_60px_rgba(123,97,255,0.1),0_0_80px_rgba(255,122,0,0.05)]">
      <motion.div
        onMouseMove={onMouseMove}
        className="group relative bg-[#0F1115] text-white rounded-[24px] border border-white/10 p-12 overflow-hidden"
      >
        {/* Spotlight effect */}
        <motion.div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center relative z-10">
          {/* IMAGE AREA */}
          <div className="relative h-[300px] flex items-center justify-center rounded-2xl border border-white/[0.08] bg-white/[0.01] overflow-hidden">
            {imageSrc ? (
              <>
          ...
          <div className="flex flex-col space-y-6 text-left">
            ...
            <div className="flex flex-wrap gap-3 pt-4">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="px-5 py-2 rounded-lg bg-white/[0.03] border border-white/[0.08] text-[10px] font-bold text-zinc-400 tracking-[0.2em] uppercase transition-all duration-500 group-hover:border-blue-500/20 group-hover:text-zinc-300"
                >
                  {tag}
                </span>
              ))}
