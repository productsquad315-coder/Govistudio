...
function getPageTemplate(folderName, isRoot = false, rootSlug = '') {
  ...
  const folderPathStr = isRoot ? '' : `'${folderName}', `;
...
export default async function Page(${isRoot ? '' : '{\n  params\n}: {\n  params: Promise<{ slug: string }>;\n}'}) {
  ${isRoot ? '' : 'const resolvedParams = await params;'}
  const slug = ${slugParam};
  
  const targetPath = path.join(process.cwd(), 'content', ${folderPathStr} slug + '.md');
...
function run() {
  ...
  const feeds = rawText.split(/### FEED \d+/).slice(1); // skip the preamble
  ...
  for (const feed of feeds) {
    ...
    let titleLine = lines.find(l => l.includes('**Feed Title:**')) || '';
    let slugLine = lines.find(l => l.includes('**URL Slug:**') || l.includes('**Slug:**')) || '';
    
    if (!slugLine) continue;

    const title = titleLine.replace(/\*\*Feed Title:\*\*/, '').trim();
    let rawSlug = slugLine.replace(/\*\*URL Slug:\*\*/, '').replace(/\*\*Slug:\*\*/, '').trim().replace(/`/g, '');
    ...
    if (parts.length === 1) {
      ...
      folderName = parts[0];
      itemSlug = parts.slice(1).join('-'); // combine remaining just in case
      routePrefixes.add(folderName);
    }

    // Prepare content dir
    const contentDirPath = isRoot ? CONTENT_DIR : path.join(CONTENT_DIR, folderName);
    ...
    let mdContent = "# " + title + "\n\n";
    let body = feed;
    // Strip the metadata block up to **Difficulty** to clean it up, or format it
    // Actually, keeping the metadata as markdown is fine, let's just use the whole feed body
    // but maybe clean up some Markdown so it renders nicely
    mdContent += body;

    // Save MD file
    const mdFilePath = path.join(contentDirPath, itemSlug + '.md');
    fs.writeFileSync(mdFilePath, mdContent);
    generatedCount++;

    // Create Next.js Route for Root level items
    if (isRoot) {
      const appRouteDir = path.join(APP_DIR, itemSlug);
      if (!fs.existsSync(appRouteDir)) fs.mkdirSync(appRouteDir, { recursive: true });
      const pageTsxPath = path.join(appRouteDir, 'page.tsx');
      fs.writeFileSync(pageTsxPath, getPageTemplate('', true, itemSlug));
    }
  }

  // Create Next.js Routes for folder items (e.g. /compare/[slug]/page.tsx)
  for (const folder of routePrefixes) {
    if (folder === 'solutions' || folder === 'blog' || folder === 'ai' || folder === 'case-studies' || folder === 'use-cases') {
        // Some might already exist and be custom
        // We'll only create [slug]/page.tsx if the base folder didn't have it, or we just overwrite generic ones
        const slugDir = path.join(APP_DIR, folder, '[slug]');
        if (!fs.existsSync(slugDir)) {
           fs.mkdirSync(slugDir, { recursive: true });
        }
        const pageTsxPath = path.join(slugDir, 'page.tsx');
        if (!fs.existsSync(pageTsxPath)) {
            fs.writeFileSync(pageTsxPath, getPageTemplate(folder, false));
        }
    } else {
        const slugDir = path.join(APP_DIR, folder, '[slug]');
        if (!fs.existsSync(slugDir)) fs.mkdirSync(slugDir, { recursive: true });
        const pageTsxPath = path.join(slugDir, 'page.tsx');
        // Always write the template to make sure it exists
        fs.writeFileSync(pageTsxPath, getPageTemplate(folder, false));
    }
  }

  console.log('Successfully generated ' + generatedCount + ' feeds and corresponding routes.');
}
